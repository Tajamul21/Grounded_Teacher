  ...
  def __getitem__(self, index):
    ...
    if self.training:
                ...
                if min_x == 0:
                    ...
                    if (box_region-trim_size) < 0:
                        ...
                        if x_s_add == 0:
                            ...
                            # if the min_x and max_x is very big 
                            valid_range = range(min_x, min(min_x + x_s_add, data.shape[2] - trim_size))
                            if valid_range:
                                x_s = np.random.choice(valid_range)
                            else:
                                x_s = max(0, data.shape[2] - trim_size)  # Ensure valid range
                # crop the image
                # print(f"data here {data.shape} ")
                # print(f"maxmin {max_x},{min_x} trim_size {trim_size} x_s {x_s}")
        ...
        if ratio < 1:
            ...
            if data.shape[2]==0:
                print("Error while data leading")
