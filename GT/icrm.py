...
class ICRm:
    def __init__(self,num_classes = 8, max_save = 50, dir = "", blocked_classes = [],mix_ratio=0.5, cfg = None):
        ...
        if 'last_checkpoint' in os.listdir(dir):
            with open(f'{dir}/last_checkpoint') as f:
                ...
                print(f"Last Checkpoint hit at: {dir}/last_checkpoint and self.dir {self.dir}")
                try:
                    self.iter = iter+1
    ...
    def get_matches(self, proposals_predictions,iter, target=False):
        ...
        # print(f"mask shape: {mask.shape}, tensor shape: {class_tensor.shape}, alpha: {alpha}")
        # print(f"mask: {mask}, tensor shape: {class_tensor}")
        # print(f"class_info: {self.class_info}")
        if target:
